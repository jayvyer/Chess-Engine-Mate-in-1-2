...
int main(int argc, char *argv[]) {
	...
	while(fgets(str, 100, fp) != NULL){
		parse_board(str);
		if(mode == 1){
			if(run_mate1(&soln) == TRUE){
				char move[] = {FILE_OF(soln.from), RANK_OF(soln.from), '-', FILE_OF(soln.to), RANK_OF(soln.to), '\n', '\0'};
				printf("Solution found: %s", move);
				fputs(move, fptr);
			}
			else{
				char notfoundmsg[] = "Solution not found\n";
				fputs(notfoundmsg, fptr);
				printf("Solution not found\n");
			}
		}
		if(mode == 2) {
			if(run_mate2(&soln) == TRUE){
				char move[] = {FILE_OF(soln.from), RANK_OF(soln.from), '-', FILE_OF(soln.to), RANK_OF(soln.to), '\n', '\0'};
				printf("Solution found: %s", move);
				fputs(move, fptr);
			}
